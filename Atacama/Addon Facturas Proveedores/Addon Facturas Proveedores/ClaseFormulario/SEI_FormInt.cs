...
using static Addon_Facturas_Proveedores.Comunes.Referen;
using static Addon_Facturas_Proveedores.Comunes.FuncionesComunes;
...
namespace Addon_Facturas_Proveedores.ClaseFormulario
{
    public class SEI_FormIntegracion
    {
        ...
        private void CargarFormulario(String FormUID)
        {
            ...
            dt.Columns.Add("co_VerPDF", SAPbouiCOM.BoFieldsType.ft_AlphaNumeric); /*Ver PDF*/ 
            ...
            dt.Columns.Add("co_TIva", SAPbouiCOM.BoFieldsType.ft_AlphaNumeric);
            dt.Columns.Add("co_neto", SAPbouiCOM.BoFieldsType.ft_AlphaNumeric);
            ...
            //Ver PDF
            oColumn = oColumns.Add("co_VerPDF", SAPbouiCOM.BoFormItemTypes.it_LINKED_BUTTON);
            oColumn.TitleObject.Caption = "Ver PDF";
            oColumn.Editable = false;
            oColumn.Visible = true;
            oColumn.Width = 45;
            oColumn.DataBind.Bind("DOCUMENTOS", "co_VerPDF");
            ...
            oColumn = oColumns.Add("co_TIva", SAPbouiCOM.BoFormItemTypes.it_EDIT);
            oColumn.TitleObject.Caption = "Monto Iva";
            oColumn.Editable = false;
            oColumn.Width = 80;
            oColumn.DataBind.Bind("DOCUMENTOS", "co_TIVA");

            oColumn = oColumns.Add("co_neto", SAPbouiCOM.BoFormItemTypes.it_EDIT);
            oColumn.TitleObject.Caption = "Monto neto";
            oColumn.Editable = false;
            oColumn.Width = 80;
            oColumn.DataBind.Bind("DOCUMENTOS", "co_neto");
                ...
                if (pVal.BeforeAction)
                {
                    switch(pVal.EventType)
                    {
                        ...
                        case SAPbouiCOM.BoEventTypes.et_MATRIX_LINK_PRESSED:
                            ...
                            GestionarVisualizar(oForm, pVal.Row);
                            break;
        ...
        public static Refern DocumetEntraOrdenCompra(String RefOC)
        {
            try
            {
                String EntradaDocu = "";
                SAPbobsCOM.Recordset oConsul = null;
                oConsul = Conexion_SBO.m_oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                EntradaDocu = @"SELECT T0.""BaseDocNum"",  SUM(T0.""PriceAfVAT"") AS TOTAL FROM PDN1 T0 WHERE T0.""BaseDocNum"" = ('" + RefOC + @"') GROUP BY T0.""BaseDocNum""";
                Refern oreferncia = new Refern();
                oConsul.DoQuery(EntradaDocu);

                {

                    oreferncia.referencia = Convert.ToString(oConsul.Fields.Item(0).Value);
                    string monto = Convert.ToString(oConsul.Fields.Item(1).Value);
                    oreferncia.monto = Convert.ToDouble(monto);
        ...
        public static void GestionarDescarga(SAPbouiCOM.Matrix oMatrix, SAPbouiCOM.Form oForm)
        {
            ...
            try
            {
                ...
                requestDescarga.AddHeader("campos", "tipoDocumento,folio,fechaEmision,fechaRecepcion,formaDePago,rutEmisor,razonSocialEmisor,iva,montoTotal,estadoSii,plazo,fechaRecepcionSii");
                                ...
                                if (ValidarFormatDesca(dto))
                                {
                                    if (!BuscarFacturasProveedores(dto.rutEmisor, dto.folio.ToString()))
                                    {
                                        if (!dto.tipoDocumento.Equals(39) && !dto.tipoDocumento.Equals(41) && !dto.tipoDocumento.Equals(52))
                                        {
                                            ...
                                            dtDoc.SetValue("co_VerPDF", IndexMatrix, "PDF");
                                            ...
                                            dtDoc.SetValue("co_TIva", IndexMatrix, String.Format(System.Globalization.CultureInfo.GetCultureInfo("es-CL"), "{0:C0}", dto.iva));
                                            dtDoc.SetValue("co_neto", IndexMatrix, String.Format(System.Globalization.CultureInfo.GetCultureInfo("es-CL"), "{0:C0}",(dto.montoTotal - dto.iva)));
                                                        ...
                                                        if (objDTE.Referencia.Count > 0)
                                                        {
                                                            ...
                                                            if (RefOC.Count > 0)
                                                            {
                                                                ...
                                                                if (existe)
                                                                {
                                                                    ...
                                                                    if (RefEM.Capacity == 0 ) {
                                                                        Refern oreferncia = new Refern();
                                                                        oreferncia =  DocumetEntraOrdenCompra(FoliosOC);
                                                                        dtDoc.SetValue("co_Exisem", IndexMatrix, "Y");
                                                                        dtDoc.SetValue("co_Folioem", IndexMatrix, oreferncia.referencia);
                                                                        dtDoc.SetValue("co_Totalem", IndexMatrix, String.Format(System.Globalization.CultureInfo.GetCultureInfo("es-CL"), "{0:C0}", Convert.ToDouble(oreferncia.monto)));
                                                            ...
                                                            if (RefOC.Count > 0)
                                                            {
                                                                ...
                                                                if (RefEM.Count.Equals(0))
                                                                {
                                                                    docRef dr = new docRef();
                                                                    dr =  buscarCompras(FoliosOC);
                                                                    dtDoc.SetValue("co_Exisem", IndexMatrix, "Y");
                                                                    dtDoc.SetValue("co_Folioem", IndexMatrix, dr.folio);
                                                                    dtDoc.SetValue("co_Totalem", IndexMatrix, String.Format(System.Globalization.CultureInfo.GetCultureInfo("es-CL"), "{0:C0}", Convert.ToDouble(dr.total)));
        ...
        public static void GestionarIntegracion(SAPbouiCOM.Form oForm)
        {
            ...
            oRecordset.DoQuery("SELECT \"U_AcepF\" FROM \"@SEI_SETVALH\"");
            blAcepComer = (oRecordset.Fields.Item("U_AcepF").Value == "Y" ? true : false);
        ...
        private static bool GestionarValidacionesFactura(SAPbouiCOM.Form oForm, Int32 index, out String mensaje, out bool noRechazable)
        {
            ...
            try
            {
                ...
                if (Conexion_SBO.m_oCompany.DbServerType == SAPbobsCOM.BoDataServerTypes.dst_HANADB)
                {

                    maxmin = @"SELECT T0.""U_Maximo"", T0.""U_Minimo"" FROM ""ESIGN_TEST"".""@SEI_SETVALL2""  T0 LIMIT 1";
                }
                else
                {
                    maxmin = @"SELECT T0.""U_Maximo"", T0.""U_Minimo"" FROM ""ESIGN_TEST"".""@SEI_SETVALL2""  T0 TOP 1";
                }
                oConsul.DoQuery(maxmin);
                max = oConsul.Fields.Item("U_Maximo").Value;
                min = oConsul.Fields.Item("U_Minimo").Value;
                montoDif = Convert.ToDouble(TotalFactura) - Convert.ToDouble(TotalEM);
        ...
        private static void IntegracionPorDocumentoBase(SAPbouiCOM.Form oForm, Int32 index)
        {
            ...
            try
            {
                ...
                String query2 = @"SELECT T0.""U_AcepF"", T0.""U_AcepE"" FROM ""@SEI_SETVALH""  T0";

                oRec.DoQuery(query2);
                blAcepComer = (oRec.Fields.Item("U_AcepF").Value == "Y" ? true : false);
                AcepEntrMerc = (oRec.Fields.Item("U_AcepE").Value == "Y" ? true : false);
                ...
                if (rslt.Success)
                {
                    ...
                    if (objDTE != null)
                    {
                        ...
                        if (RetVal.Equals(0))
                        {
                            ...
                            if (AcepEntrMerc)
                            {
                                rslt = FuncionesComunes.EnviarRespuestaComercialEntradaMerca(FebId, "ERM", String.Empty, String.Empty);
                                if (rslt.Success)
                                {
                                    Conexion_SBO.m_SBO_Appl.StatusBar.SetText(String.Format("Exito: El DTE {0} Tipo {1} de {2}-{3} Se integro.", objDTE.IdDoc.Folio, objDTE.IdDoc.TipoDTE, objDTE.Emisor.RznSoc, objDTE.Emisor.RUTEmisor), SAPbouiCOM.BoMessageTime.bmt_Long, SAPbouiCOM.BoStatusBarMessageType.smt_Success);
                                    //file.WriteLine(String.Format("Exito: El DTE {0} Tipo {1} de {2}-{3} Se integro.", objDTE.IdDoc.Folio, objDTE.IdDoc.TipoDTE, objDTE.Emisor.RznSoc, objDTE.Emisor.RUTEmisor));
            ...
            try
            {
                ...
                for (int index=1;index<= oMatrix.VisualRowCount;index++)
                {
                    ...
                    if (oCheckBox.Checked)
                    {
                        ...
                        doc.MntTotal = objDTE.Totales.MntTotal.ToString();
                        doc.IVA = objDTE.Totales.IVA.ToString();
                        doc.Neto = objDTE.Totales.MntNeto.ToString();
                        ListDoc.Add(doc);
                    }
                }               
