...
namespace Addon_Facturas_Proveedores.Comunes
{
    ...
    public class FuncionesComunes
    {
        ...
        public static ResultMessage EnviarRespuestaComercialEntradaMerca(String FebosID, String TipoAccion, String Motivo, String Recinto)
        {
            ResultMessage result = new ResultMessage();
            result.Success = false;
            ...
            if (response.StatusDescription.Equals("OK"))
            {
                if (TipoAccion.Equals("ERM"))
                {
                    result = EnviarRespuestaComercialEntradaMerca(FebosID, "ERM", String.Empty, ObtenerRecinto());
                }
                else
                {
                    result.Success = true;
        ...
        public static docRef buscarCompras(string doc_Num)
        {
            ...
            try
            {
                SAPbobsCOM.Recordset oRecordset = null;
                oRecordset = Conexion_SBO.m_oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
                string query = "SELECT T2.\"DocEntry\", SUM (T2.\"LineTotal\" ) AS \"Total\"FROM OPOR T0 INNER JOIN POR1 T1 ON T0.\"DocEntry\" = T1.\"DocEntry\"INNER JOIN PDN1 T2 ON T2.\"BaseEntry\" = T1.\"DocEntry\" AND T2.\"BaseLine\" = T1.\"LineNum\"INNER JOIN OPDN T3 ON T2.\"DocEntry\" = T3.\"DocEntry\" WHERE T0.\"DocNum\" = '" + doc_Num + "' GROUP BY T2.\"DocEntry\"";
                oRecordset.DoQuery("SELECT T2.\"DocEntry\", SUM (T2.\"LineTotal\" ) AS \"Total\"FROM OPOR T0 INNER JOIN POR1 T1 ON T0.\"DocEntry\" = T1.\"DocEntry\"INNER JOIN PDN1 T2 ON T2.\"BaseEntry\" = T1.\"DocEntry\" AND T2.\"BaseLine\" = T1.\"LineNum\"INNER JOIN OPDN T3 ON T2.\"DocEntry\" = T3.\"DocEntry\" WHERE T0.\"DocNum\" = '" + doc_Num + "' GROUP BY T2.\"DocEntry\"");
                docRef dr = new docRef();
                dr.folio = Convert.ToString(oRecordset.Fields.Item(0).Value);
                Double.TryParse(oRecordset.Fields.Item(1).Value.ToString(), NumberStyles.Any, CultureInfo.InvariantCulture, out dr.total);
                return dr;
